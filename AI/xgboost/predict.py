...
# Load models with joblib - no pickle issues!
yield_model = joblib.load("models/yield_model.joblib")
quality_model = joblib.load("models/quality_model.joblib")
crop_enc = joblib.load("models/crop_encoder.joblib")
type_enc = joblib.load("models/type_encoder.joblib")
quality_enc = joblib.load("models/quality_encoder.joblib")
FEATURES = joblib.load("models/features.joblib")  # Also convert this if not already
...
def fetch_climate(lat: float, lon: float, year: int) -> dict:
    ...
    try:
        ...
        return {"avg_temp": 22.0, "total_rain": 250.0, "avg_humidity": 55.0, "avg_radiation": 18.0}
